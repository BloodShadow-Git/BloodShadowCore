namespace BloodShadow.Core.OrGraph
{
    ...
    public class Tree<T> : IReadOnlyTree<T> where T : notnull
    {
        ...
        public Tree(Func<T, T, T>? unitFunc)
        {
            ...
            Pairs = new List<KeyValuePair<T, T>>();
            Layers = new List<List<T>>();
            EndElements = new List<T>();
            Elements = new List<T>();
            _unitDictionary = new Dictionary<T, List<T>>();
        }
        ...
        public Tree(Tree<T> one, Tree<T> two)
        {
            ...
            Elements = one.Elements.Concat(two.Elements).ToList();
            Pairs = one.Pairs.Concat(two.Pairs).ToList();
            Layers = new List<List<T>>();
            EndElements = new List<T>();
            _unitDictionary = new Dictionary<T, List<T>>();
        ...
        public void Add(KeyValuePair<T, T> pair)
        {
            ...
            Elements = (from search in Elements where search != null select search).Distinct().ToList();
        ...
        {
            List<KeyValuePair<T, T>> _pairs = new List<KeyValuePair<T, T>>(Pairs);
            foreach (KeyValuePair<T, T> pair in Pairs)
            {
                if (pair.Value == null) { continue; }
                List<T> checkedElements = new List<T>();
                List<KeyValuePair<T, T>> visited = new List<KeyValuePair<T, T>>();
                Stack<KeyValuePair<T, T>> frontier = new Stack<KeyValuePair<T, T>>();
                ...
                while (frontier.Count > 0)
                {
                    ...
                    newCycle:;
                        ...
                        List<KeyValuePair<T, T>> toRemove = new List<KeyValuePair<T, T>>();
                        ...
                        visited = visited.Except(checkedPairs).ToList();
                        ...
                        depends = depends.Distinct().Except(elements).ToList();
        ...
        {
            Layers.Clear();
            Queue<T> elementsToAdd = new Queue<T>(Elements);
            List<T> addedElements = new List<T>();
            while (elementsToAdd.Count > 0)
            {
                ...
                if (elementPairs.Count != 0)
                {
                    if (elementPairs.Count == 1)
                    {
                        ...
                        {
                            if (Layers.Count == 0) { Layers.Add(new List<T>()); }
                    ...
                    if (max >= Layers.Count - 1) { Layers.Add(new List<T>() { element }); }
        ...
        {
            EndElements.Clear();
            Queue<T> queue = new Queue<T>(Elements);
            while (queue.Count > 0)
            {

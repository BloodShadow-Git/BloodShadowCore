...
{
    public class OrGraph<T> : IReadOnlyOrGraph<T> where T : notnull
    {
        ...
        public OrGraph() { Trees = new List<Tree<T>>(); }
        ...
        {
            List<Tree<T>> toRemove = new List<Tree<T>>();
            List<Tree<T>> toAdd = new List<Tree<T>>();
            ...
            foreach (Tree<T> first in Trees)
            {
                foreach (Tree<T> second in Trees)
                {
                    ...
                    {
                        Tree<T> tree = new Tree<T>(first, second);
                        tree.Add(pair);
                        toRemove.Add(first);
                        toRemove.Add(second);
                        toAdd.Add(tree);
        ...
        exit:;
            ...
            Tree<T> _tree = new Tree<T>(_uniteFunction);
            _tree.Add(pair);
            Trees.Add(_tree);
        }
        ...
        public T[] GetItems(int index)
        {
            ...
            for (int i = 0; i < Trees.Count; i++)
            {
                ...
                {
                    if (i == 0) { result = Trees[i].Layers.ElementAt(index).ToArray(); }
                    else { result = result.Union(Trees[i].Layers.ElementAt(index)).ToArray(); }
                }
            }
            return result;
        }
        ...
        {
            if (Trees.Count > tree) { if (Trees[tree].Layers.Count > index) { return Trees[tree].Layers.ElementAt(index).ToArray(); } }

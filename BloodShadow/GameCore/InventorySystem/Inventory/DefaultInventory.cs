...
namespace BloodShadow.GameCore.InventorySystem.Inventory
{
    public class DefaultInventory : Inventory
    {
        ...
        private readonly ObservableList<InventoryData> _items = new ObservableList<InventoryData>();

        public DefaultInventory(string key) { _localizationKey = key; }
        [JsonConstructor]
        public DefaultInventory(string key, IEnumerable<Item> items) : this(key) { _items = new ObservableList<InventoryData>(items.Cast<InventoryData>()); }
        ...
        private void AddStackable(Item item, int count)
        {
            ...
            if (itemIndex == -1) { _items.Add(new InventoryData(item, count)); }

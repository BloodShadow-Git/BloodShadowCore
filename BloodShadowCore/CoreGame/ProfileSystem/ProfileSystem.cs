namespace BloodShadow.CoreGame.ProfileSystem
{
    ...
    public class ProfileSystem
    {
        ...
        public readonly string ProfilesCombinedPath;
        ...
        public ProfileSystem(string dataPath, string tempPath, string savePath, string runtimeData, string profilesDir)
            : this(dataPath, tempPath, savePath, runtimeData, profilesDir, new JsonSaveSystem()) { }
        public ProfileSystem(string dataPath, string tempPath, string savePath, string runtimeData, string profilesDir, SaveSystem saveSystem)
        {
            _saveSystem = saveSystem;
            _active = new();
            _profiles = [];

            ProfilesPath = Path.Combine(savePath, runtimeData, profilesDir);
            ProfilesCombinedPath = Path.Combine(dataPath, ProfilesPath);
            TempProfilesPath = Path.Combine(tempPath, ProfilesPath);
            _saveSystem.CheckFolder(ProfilesCombinedPath);
            _saveSystem.CheckFolder(TempProfilesPath);

            _profileWatcher = new(ProfilesCombinedPath) { IncludeSubdirectories = false };
        ...
        public bool CreateProfile(string name, byte[] icon, IDictionary<Type, object> dict, out Profile profile)
        {
        ...
        public bool RemoveProfile(Profile profile)
        {
            ...
            if (string.IsNullOrEmpty(profile.Name)) { return false; }
            if (!ProfileNames.Contains(profile.Name)) { return false; }
            File.Delete(Path.Combine(ProfilesCombinedPath, GenerateFileName(profile)));
            UpdateProfiles();
            return true;
        }
        ...
        {
            _profiles.Clear();
            IEnumerable<FileInfo> profilesFiles = Directory.EnumerateFiles(ProfilesCombinedPath, EXTENSIONFILTER).Select(input => new FileInfo(input));

        ...
        public static async Task<UnitTestResult[]> StartTest()
        {
            ...
            for (int assemblyIndex = 0; assemblyIndex < assemblies.Length; assemblyIndex++)
            {
                ...
                for (int typeIndex = 0; typeIndex < types.Length; typeIndex++)
                {
                    ...
                    {
                        UnitTestAttribute[] tests = [.. methods[methodIndex].GetCustomAttributes<UnitTestAttribute>(true)];
                        ...
                        for (int testIndex = 0; testIndex < tests.Length; testIndex++)
                        {
                            if (CheckParameters(methods[methodIndex].GetParameters(), tests[testIndex].Arguments))
                            {
                                try
                                {
                                    if ((bool)methods[methodIndex].Invoke(null, [.. tests[testIndex].Arguments]))
                                    {
